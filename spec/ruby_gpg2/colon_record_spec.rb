...
describe RubyGPG2::ColonRecord do
  describe 'parsing' do
    describe 'for example records' do
      ...
      it 'parses a fingerprint record' do
        ...
        key_list_record = described_class.parse(colon_record)
    ...
    describe 'for key algorithms' do
      {
    ...
    describe 'for key capabilities' do
      {
        'e' => :encrypt,
        's' => :sign,
        'c' => :certify,
    ...
    describe 'for compliance modes' do
      {
        '8' => :rfc_4880bis,
        '23' => :de_vs,
  ...
  describe 'properties' do
    it 'uses the raw record from the provided options' do
      key_list_record = described_class.new(
        raw: 'pub:u:2048:1:1A16916844CE9D82:' \
             '1333003000:::u:::scESC::::::23::0:'
      )
  ...
  describe 'user ID methods' do
    describe 'user_id_record?' do
      it 'returns true when record is a user_id' do
        colon_record = described_class.new(
          raw: 'uid:u::::1333003000::' \
               '6D9560936837E5AC5007524183B9F354E4F5308C' \
               '::Toby Clemson <tobyclemson@gmail.com>::::::::::0:',
          type: :user_id,
  ...
  describe 'equality' do
    it 'is equal when all properties are equal' do
      key_list_record1 = build_record
      key_list_record2 = build_record

      expect(key_list_record1)
        .to(eq(key_list_record2))
    end

    it 'is not equal if type is different' do
      key_list_record1 = build_record(type: :public_key)
      key_list_record2 = build_record(type: :secondary_key)

      expect(key_list_record1)
        .not_to(eq(key_list_record2))
    end

    it 'is not equal if trust model is different' do
      key_list_record1 = build_record(trust_model: :classic)
      key_list_record2 = build_record(trust_model: :pgp)
    ...
    it 'is not equal if validity is different' do
      key_list_record1 = build_record(validity: :ultimate)
      key_list_record2 = build_record(validity: :revoked)
    ...
    it 'is not equal if key length is different' do
      key_list_record1 = build_record(key_length: 1024)
      key_list_record2 = build_record(key_length: 2048)
    ...
    it 'is not equal if compliance modes are different' do
      ...
      key_list_record2 = build_record(
        compliance_modes: %i[de_vs rfc_4880bis]
      )
  ...
  def build_record(overrides = {})
    RubyGPG2::ColonRecord.new({
      raw: 'pub:u:2048:1:1A16916844CE9D82:1333003000:::u:::scESC::::::23::0:',
      type: :public_key,
      validity: :ultimate,
      key_length: 2048,
      key_algorithm: :rsa_encrypt_or_sign,
      ...
      owner_trust: :ultimate,
      key_capabilities:
        %i[sign certify primary_encrypt primary_sign primary_certify],
      compliance_modes: [:de_vs],
      last_update: :unknown,
      origin: '0'
    }.merge(overrides))
